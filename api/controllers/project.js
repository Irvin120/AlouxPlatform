...
self.retrieve = async (req, res) => {
  try {
    const businessId = await BusinessController.validateBusinessId(req)
    const retrieve = await Project.find({ _business: businessId })
      .populate('_client')
      .sort({ _id: -1 })
      .lean();
...
self.count = async (req, res) => {
  try {
    const businessId = await BusinessController.validateBusinessId(req)
    const response = await Project.countDocuments({ _business: businessId })

    if (count === null || count === undefined) {
      throw { status: 400, message: "Error al contar los proyectos." };
    }
